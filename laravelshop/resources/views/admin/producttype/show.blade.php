   ...
   <div class="">
      <div class="page-title">
         ...
         <div class="title_right" align="right">
           ...
           <a href="{{ route('admin.producttype.getAdd') }}" class="btn btn-success btn-sm glyphicon glyphicon-plus" >Thêmmới</a>
               ...
               <div class="x_content">
                  ...
                  <table class="table table-striped responsive-utilities jambo_table bulk_action">
                     ...
                     <tbody>
                        ...
                        <tr class="even pointer producttype{{$value->id}}">
                           <td class="a-center ">
                              <input type="checkbox" class="flat" value="{{ $value->id }}" name="table_records">
                           </td>
                           <td class=" ">{{ $value->id }}</td>
                           <td class=" ">{{ $value->name }}</td>
                           <td class=" "><img src={{ URL::to('/') }}/images/producttype/{{$value->images}} width='70' class='img-thumbnail' /></td>
...
<script type="text/javascript">
   ...
   /*======================================LaydulieuformEdit=======================================*/
     $(document).on('click', '.edit-modal', function() {
        ...
        // $('#form_result').html('');
        $.ajax({
            url: "{{ url('admin/producttype/edit')}}/" + id,
            dataType: "json",
            success: function($data) {
                var html = '';
                $.each($data.category,function($key,$value){
                  if($value['id'] == $data.producttype.category_id){
                ...
                $('#fid').val($data.producttype.id);
                $('#store_image').html("<img src={{ URL::to('/') }}/images/producttype/" + $data.producttype.images + " width='200px' class='img-thumbnail' />");
                $('#store_image').append("<input type='hidden' name='hidden_image' value='" + $data.producttype.images + "' />");
                $('#namee').val($data.producttype.name);
                $('#hidden_id').val($data.producttype.id);
...
/*======================================UpdateEdit=======================================*/
     $('.modal-footer').on('click', '.edit', function() {
           ...
           $.ajax({
               url: "{{route('admin.producttype.postEdit')}}",
               method: "POST",
               data: formData,
               contentType: false,
               cache: false,
  ...
  /*=============================================Delete==============================================*/
      ...
      $('.modal-footer').on('click', '.delete', function() {
          ...
          $.ajax({
              type: 'POST',
              url: '{{ route('admin.producttype.postDelete')}}',
              data: {
           ...
           if((id.length) > 0)
           {
              $.ajax({
                 ...
                 success : function(data){
                  toastr.success(data.success, 'Thông Báo',  {timeOut: 5000, progressBar : true })
                   $.each($("input[class='flat']:checked"), function(){            
                      $('.producttype' + $("input[class='flat']:checked").val()).remove();
                   });
                 }
              });
           }
